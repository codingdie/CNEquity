...
| cninfo / pboc 间歇失败 | 同源风控 / 站点抖动 | 已按页/按调用 `rate_limit`；社融主写入按年取全量，任一年失败即阻止本次宏观写入，待下次完整重试 |
...
| `cne status --datasets` valuation STALE + 覆盖 ~20% | 同上 | 对缺口日补东财快照（PK `keep=last` 覆盖稀疏 baostock）： |

```bash
# 例：2026-07-17 … 最近交易日（按实际缺口改）
for d in 2026-07-17 2026-07-18 2026-07-21 2026-07-22 2026-07-23 2026-07-24 2026-07-25; do
  uv run cne run daily --group capital --trade-date "$d"
done
uv run cne status --datasets   # valuation 不应再停在稀疏 tip
```
...
- **处理**：可接受则继续；要纯 TDX tip 时修好 TDX 后对当日 `cne run daily --group core --trade-date …` 重跑（compact `keep=last` 会用更新的主源行覆盖同 PK）。
- **多日回填**失败仍走 kline gap-fill（慢）；clist **不能**伪造历史。

### 周末 / 漏跑后水位落后

- **现象**：今天非交易日时 `cne run daily` → `skipped_non_trading_day`；`daily_bars`
...
- **处理**：提高 `[sources.ths] min_interval_seconds`（默认 1.0，实测这个速率下
  ~1300 次连续请求零失败），再 `cne backfill sector_bars --retry-failed`；
...
```bash
# 本机 → VPS（示例）
rsync -avz --progress ~/code/cnequity/ user@VPS:~/cnequity/
...
# 跑完把湖同步回本机
rsync -avz --progress user@VPS:~/cnequity/data/cnequity/ \
  ~/code/cnequity/data/cnequity/
```
...
2. 查该数据集最近成功 run：`cne status`
3. 重跑对应 group：`cne run daily --group <name>`
