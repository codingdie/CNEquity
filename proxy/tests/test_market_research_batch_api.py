...
def test_market_research_batches_stream_original_parquet_files(
    lake_root: Path,
    client: TestClient,
    path: str,
    archive_name: str,
    ...
    source_paths = sorted((lake_root / archive_name).glob("trade_date=*/*.parquet"))
...
def test_trading_status_batch_selects_overlapping_month_files_without_reencoding(
    lake_root: Path,
    client: TestClient,
):
    response = client.get(
    ...
    assert response.status_code == 200
    name = "curated/trading_status/trade_date=2026-01/part-merged.parquet"
    with tarfile.open(fileobj=io.BytesIO(response.content), mode="r:") as archive:
...
@pytest.mark.parametrize(
    "path",
    ["/v1/adjustment-factors/batch", "/v1/trading-status/batch"],
)
def test_market_research_batches_validate_windows_but_have_no_batch_quotas(
    ...
    assert response.status_code == 200
    assert client.get(path, params={"start": "2026-02-01", "end": "2026-02-02"}).status_code == 404
...
def test_market_research_batches_bypass_regular_query_quota(settings: ProxySettings, path: str):
...
def test_market_research_batches_require_authentication_and_never_write_to_the_lake(
    lake_root: Path,
    settings: ProxySettings,
    path: str,
):
