...
def _bars_missing_factor_partitions(
    ...
    *,
    ...
    bars = _load_daily_bar_dates(config, start=start).filter(pl.col("trade_date") <= watermark)
    if bars.is_empty():
        return bars
    ...
    for trade_date in bars["trade_date"].unique().sort().to_list():
        part = config.derived_root / "adj_factors" / f"trade_date={trade_date.isoformat()}"
