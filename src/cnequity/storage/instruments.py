...
_BAOSTOCK_FORMAL_EXCHANGES = ("SH", "SZ")
...
def _formal_delist_dates(curated_root: Path, as_of: date) -> dict[str, date] | None:
    """Read the complete SH/SZ security-master delisting identity observation.
    ...
    consecutive snapshots omit the symbol. Baostock's full SH/SZ
    security-master evidence is deliberately separate from instruments so
    compact can verify a sticky ``delist_date`` instead of treating an old
    inference as fact.
    """
    path = (
        curated_root.parent
...
def _apply_formal_delist_dates(df: pl.DataFrame, formal_dates: dict[str, date]) -> pl.DataFrame:
    """Reconcile SH/SZ dates against the independently certified master.

    Baostock's security master does not cover BJ names, so it cannot clear a
    terminal date established by the explicit BJ recovery workflow.
    """
    ...
    return (
        df.join(dates, on="symbol", how="left")
        .with_columns(
            pl.when(pl.col("exchange").is_in(_BAOSTOCK_FORMAL_EXCHANGES))
            .then(pl.col("_formal_delist_date"))
            .otherwise(pl.col("delist_date"))
            .alias("delist_date")
        )
        .drop("_formal_delist_date")
    )
...
def compact_instruments(
    ...
    # Baostock SH/SZ security-master evidence and thereby repair those old
    # claims. BJ dates retain their explicit recovery provenance because this
    # source has no authority over that market.
    ...
    if delist_identity_changed is not None:
        ...
        delist_identity_changed.update(
            ...
            if symbol.rpartition(".")[2] in _BAOSTOCK_FORMAL_EXCHANGES
            and prior_delist_dates.get(symbol) != current_delist_dates.get(symbol)
        )
