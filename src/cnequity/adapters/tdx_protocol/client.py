...
def _pick_reachable_server(config: Config | None = None, timeout: int = 10) -> tuple[str, int]:
    ...
    for offset in range(0, len(candidates), _TDX_MAX_CANDIDATES):
        ...
        pool = ThreadPoolExecutor(max_workers=min(len(batch), _TDX_PROBE_CONCURRENCY))
        ...
        try:
            ...
            for fut in futures:
                fut.cancel()
            # The context-manager form always waits for in-flight probes. A
            # winning server must be returned promptly; abandoned probes have
            # bounded socket/request timeouts and release their own leases.
            pool.shutdown(wait=winner is None, cancel_futures=True)
