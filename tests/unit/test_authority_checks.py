...
def _lake_with_instruments(
    ...
    *,
    ...
    delist_date: date = date(2025, 1, 1),
) -> Config:
    ...
    pl.DataFrame(
        {
            ...
            "delist_date": [delist_date if symbol in delisted else None for symbol in symbols],
...
def test_delisted_st_names_are_not_a_shortfall(monkeypatch, tmp_path):
    ...
    _exchange(monkeypatch, names)
    status = {s: "normal" for s in syms}
    cfg = _lake_with_instruments(tmp_path, status=status, delisted=delisted, delist_date=TD)
    assert ac.st_labels_vs_exchange(cfg, TD) == []
