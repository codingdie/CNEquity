...
def test_compact_instruments_keeps_bj_recovery_date_outside_baostock_scope(tmp_path):
    cfg = Config(data_root=tmp_path / "data")
    curated_path = cfg.curated_root / "instruments" / "part-merged.parquet"
    curated_path.parent.mkdir(parents=True)
    pl.DataFrame(
        [
            _instrument("600519.SH"),
            _instrument("920001.BJ", delist_date=date(2024, 6, 20)),
        ]
    ).write_parquet(curated_path)
    _write_formal_delist_identity(cfg, {"600001.SH": date(2009, 12, 25)})
    ...
    StagingWriter(cfg.staging_root).write_batch(
        "instruments",
        "run-preserve-bj-recovery",
        "batch-0",
        pl.DataFrame([_instrument("600519.SH"), _instrument("920001.BJ")]),
    )
    compact_instruments(
        cfg.staging_root,
        cfg.curated_root,
...
def test_audit_does_not_emit_absence_based_delist_error(tmp_path):
    ...
    assert not [f for f in payload["findings"] if f.get("check") == "instruments_delist_suppressed"]
